...
func CreateJob(job Job) (bool, int) {
	var id int
	sql := `INSERT INTO jobs (user_id, company_name, title, city, state, posting_url, status, deadline_date, notes) 
				VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
				RETURNING id`
	err := DbConn.QueryRow(context.Background(), sql, job.user_id, job.company_name, job.title, job.city, job.state, job.posting_url, job.status, job.deadline_date, job.notes).Scan(&id)
	if err != nil {
